...
from ...criterions.AUCROC_RankingLoss import AUCROC_RankingLoss 
...
# t_SNE_features = torch.empty((1,256))
# t_SNE_masks = torch.empty((1,1))
...
class MaskDINOEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.mask_FPN = torchvision.ops.FeaturePyramidNetwork([256,256,256,256], 256)
        self.anomaly_map_generate = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=256, out_channels=1, kernel_size=1),
            torch.nn.Sigmoid()
        )
        self.AUCROC_RankingLoss = AUCROC_RankingLoss(num_classes = 1, num_anchors = 6, fpr_range_lower = 0.0, fpr_range_upper = 0.3, device = 'cuda:0')
    ...
    def forward_features(self, features, masks):
        ...
        """
        # global t_SNE_masks, t_SNE_features
        # backbone features
        srcs = []
        pos = []
        ...
        #--------------------------------------------------------------------------------
        for idx, f in enumerate(self.in_features[:self.num_fpn_levels][::-1]):# 和mask2former一致，也是融合backbone最底层size256的特征和pixel_encoder输出的size 128的特征
            x = features[f].float() #torch.Size([2, 256, 256, 256])
        ...
        fpn_input['feat0'] = features['res2'].float()
        fpn_input['feat1'] = out[0]
        fpn_input['feat2'] = out[1]
        fpn_input['feat3'] = out[2]

        # Pixel-level Anomaly Map Generation
        anomaly_feature = self.mask_FPN(fpn_input)['feat0']
        anomaly_maps = self.anomaly_map_generate(anomaly_feature)
        mask_feature = self.mask_features(out[-1])

        if masks != None:
            gt_mask = torch.nn.functional.interpolate(masks, (mask_feature.shape[2], mask_feature.shape[3])).to(mask_feature.device)
            anomaly_map_losses = self.AUCROC_RankingLoss(anomaly_maps.ravel().float(), gt_mask.ravel().int())
        else:
            anomaly_map_losses = 0.0
        #--------------------------------------------------------------------------------
        ...
        return mask_feature, anomaly_map_losses, anomaly_maps, out[0], multi_scale_features