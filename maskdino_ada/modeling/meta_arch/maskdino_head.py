...
class MaskDINOHead(nn.Module):
    ...
    def layers(self, features, mask=None,targets=None):
        mask_features, anomaly_loss, anomaly_maps, transformer_encoder_features, multi_scale_features = self.pixel_decoder.forward_features(features, mask)

        # predictions = self.predictor(multi_scale_features, mask_features, mask, targets=targets)
        predictions = self.predictor(multi_scale_features, mask_features, anomaly_maps, None, targets=targets)
        
        predictions[0]['anomaly_loss'] = 5*anomaly_loss
        predictions[0]['anomaly_maps'] = anomaly_maps
