...
"""
2023.09.28 
        mask_features = mask_features + sort_pos_emd
...
class MaskDINODecoder(nn.Module):
    ...
    def __init__(
        ...
        self.query_feat = nn.Embedding(num_queries, hidden_dim)
        self.query_embed = nn.Embedding(num_queries, 4)
        self.pose = PositionEmbeddingSine(num_pos_feats=128, normalize=True)
        self.rank_embeded_generate = torch.nn.Sequential(
    ...
    def forward(self, x, mask_features, anomaly_maps, masks, targets=None):
        """
        ...
        #------------------------------------------------------------------
        # Embedding Refinement Guided by the Anomaly Map
        anomaly_shape = anomaly_maps.shape
        pos_emd = self.pose(anomaly_maps)
        pos_emd = pos_emd.flatten(2)
        anomaly_maps = anomaly_maps.flatten(1).detach()
        _, idx = anomaly_maps.sort(dim=1, descending=True)
        _, rank = idx.sort(dim=1)
        sort_pos_emd = pos_emd[:,:,rank.squeeze(0)]
        sort_pos_emd = sort_pos_emd.reshape((sort_pos_emd.shape[0], sort_pos_emd.shape[1], anomaly_shape[2], anomaly_shape[3]))
        sort_pos_emd = self.rank_embeded_generate(sort_pos_emd)
        mask_features = mask_features + sort_pos_emd
        #------------------------------------------------------------------
        ...
        if self.two_stage:
            #---------------------------------------------------------------------------------
            # Query Selection Guided by the Anomaly Map
            bkg_index = np.random.choice(torch.where(anomaly_maps > torch.quantile(anomaly_maps, 0.7))[1].detach().cpu().numpy(), self.num_queries)
            query_feat2 = mask_features.flatten(2)[:,:,bkg_index].permute((0,2,1))
            query_feat1 = self.query_feat.weight[None].repeat(bs, 1, 1)
            tgt_undetach = query_feat1 + query_feat2
            #---------------------------------------------------------------------------------
            ...
            if self.initialize_box_type != 'no':
                ...
                #---------------------------------------------------------------------------------
                interm_outputs['pred_boxes'] = refpoint_embed
                #---------------------------------------------------------------------------------
